...
public final class RealConnection extends Http2Connection.Listener implements Connection {
    ...
    while (true) {
      try {
        if (route.requiresTunnel()) {
          ...
          connectSocket(connectTimeout, readTimeout); //安全网关链接走该分支
        }
  ...
  private void connectSocket(int connectTimeout, int readTimeout) throws IOException {
    ...
    try {
      Platform.get().connectSocket(rawSocket, route.socketAddress(), connectTimeout); //AndroidPlatform.connectSocket()
      Logger.i("链接socket信息="+rawSocket.getLocalSocketAddress());
  ...
  private void connectTls(ConnectionSpecSelector connectionSpecSelector) throws IOException {
    ...
    try {
      ...
      try {
        //开启所有支持的cipher suite,避免未启用cipher suite导致的javax.net.ssl.SSLHandshakeException
